...
train_data, test_data, train_indices, test_indices = train_test_split(data, range(len(data)), test_size=0.2, random_state=42)
...
def evaluate_model():
    ...
    for idx in test_indices:
        test_row = data.iloc[idx]
        recommendations = get_recommendations_by_input(
            harga=test_row['harga'], 
            lokasi=test_row['lokasi_jabodetabek'], 
            AC=test_row['AC'], 
            Kasur=test_row['Kasur'], 
            Lemari=test_row['Lemari'], 
            WiFi=test_row['WiFi'], 
            Wc_duduk=test_row['Wc duduk'], 
            Kamar_mandi_dalam=test_row['Kamar mandi dalam'], 
            Listrik=test_row['Listrik'],
            deskripsi=test_row['deskripsi']
        )
        
        # Check if the actual kost is in the top 5 recommendations
        y_true.append(1)
        y_pred.append(1 if idx in recommendations else 0)
