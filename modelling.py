...
additional_features = np.array(data[['lokasi_encoded', 'gender_encoded', 'harga']])
...
# Train the model
model.fit(X_train, X_train, epochs=25, batch_size=32, validation_split=0.2)

# Function to get kost recommendations based on user input
def get_recommendations_by_input(harga=None, lokasi=None, gender=None, AC=0, Kasur=0, Lemari=0, WiFi=0, Wc_duduk=0, Kamar_mandi_dalam=0, Listrik=0, deskripsi=None):
    ...
    # Encode gender if provided
    gender_encoded = le_gender.transform([gender])[0] if gender else 0

    # Construct the feature vector for the query
    query_features = np.hstack((query_tfidf.toarray(), [[lokasi_encoded, gender_encoded, harga]]))
    ...
    # Get the indices of kosts that match the specified gender
    if gender:
        gender_indices = data[data['gender'] == gender].index
    else:
        gender_indices = data.index

    # Intersect the indices of location and gender to get the final indices
    final_indices = location_indices.intersection(gender_indices)

    # Filter the similarity scores to include only kosts from the specified location and gender
    query_sim_filtered = query_sim[:, final_indices]

    # Get the top 25 most similar kosts
    query_sim_scores = list(enumerate(query_sim_filtered[0]))
    query_sim_scores = sorted(query_sim_scores, key=lambda x: x[1], reverse=True)
    query_sim_scores = query_sim_scores[:25]
    kost_indices = [final_indices[i[0]] for i in query_sim_scores]
    
    return data[['nama_kost', 'gender', 'id', 'alamat', 'harga', 'Luas kamar', 'deskripsi']].iloc[kost_indices]
